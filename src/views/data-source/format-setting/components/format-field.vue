<template>
  <div class="format-field">
    <header :class="field_data.type === 'normal'? '':'special'">
      {{field_data.location}}
      <i
        :class="`iconfont ${get_field_header_icon}`"
    ...
    <main>
      <el-input
        ...
        :title="field_label"
        v-model="field_label"
        @change="handle_change_field_label"
      ></el-input>
    </main>
    <footer>
      ...
      <div class="field_attribute">
        <div
          :class="field_data.required? 'mas-is-required':'mas-no-required'"
          @click="handle_change_field_required"
        ></div>
        <div
          :class="field_data.to_report_form? 'mas-to-reportForm':'mas-no-reportForm'"
          @click="handle_change_field_reportForm"
        ></div>
      </div>
      <div class="sort-field">
        <span class="iconfont mas-arrow-left-end" title="First" @click="handle_first"></span>
        <span class="iconfont mas-arrow-left" title="Prev" @click="handle_prev"></span>
        <span class="iconfont mas-arrow-right" title="Next" @click="handle_next"></span>
        <span class="iconfont mas-arrow-right-end" title="Last" @click="handle_last"></span>
      </div>
      <div class="delete" @click="handle_delete_field" title="Delete">
...
export default {
  ...
  methods: {
    handle_change_field_label() {
      if (this.field_label.trim() && this.field_label.trim().length > 0) {
        ...
        this.$emit("change-label", {
          createTime: this.field_data.createTime,
          label: this.field_data.label
        });
      } else {
    ...
    handle_show_dialog(type) {
      if (type === "formula") {
        this.$refs.formulaDialog.handle_update_formula_field(this.field_data);
...
.format-field {
  ...
  footer {
    ...
    .field_attribute {
      ...
      .mas-is-required {
        background: url("~@/assets/images/data_source/required_true.png")
          no-repeat center center;
        background-size: cover;
      }
      .mas-no-required {
        background: url("~@/assets/images/data_source/required_false.png")
          no-repeat center center;
        background-size: cover;
      }
      .mas-to-reportForm {
        background: url("~@/assets/images/data_source/reportForm_true.png")
          no-repeat center center;
        background-size: cover;
      }
      .mas-no-reportForm {
        background: url("~@/assets/images/data_source/reportForm_false.png")
          no-repeat center center;
        background-size: cover;
      }
    }
