...
export default {
  ...
  data() {
    return {
      ...
      groupValue: [],
      brandValue: ["Estee Lauder", "Lancome"],
  ...
  methods: {
    ...
    async init() {
      try {
        let [res_year, res_category, res_group, res_brand, res_digital, res_type] = await Promise.all([
          this.fetch_filter_list({ type: "year", value: "1" }),
          this.fetch_filter_list({ type: "category", value: "" }),
