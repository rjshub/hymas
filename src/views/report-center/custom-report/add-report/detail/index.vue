...
export default {
  ...
  components: { breadCrumb, checkboxDropdown, zoomIn },
  props: {
    values: {
      type: [Object, Array]
    }
  ...
  methods: {
    ...mapActions("reportCenter", [
      "fetch_custome_report_detail_filter",
      "fetch_custome_report_detail_table",
      "fetch_custome_report_detail_download",
    ...
    setNameAndCycle() {
      ...
      let end = this.$route.query.end == "Today" ? moment().format("YYYY-MM-DD") : this.$route.query.end;
      this.filter.date = [begin, end];

      this.cycle = [begin, this.$route.query.end];
    },
    ...
    get_product_by_brand(brand) {
      let params = { id: this.$route.query.id, brand };
    ...
    async get_filter_select_data() {
      ...
      this.allFilters = await this.fetch_custome_report_detail_filter(params);
    ...
    async get_table_list() {
      try {
        ...
        let params = {
          id: this.$route.query.id,
          search_start: this.filter.date[0],
          search_end: this.filter.date[1],
          ...this.filter.selects,
          columns: this.allColumns.length != 0 ? this.filter.selectedColumns : "all", //如果columns为all，表示全选所有的，如果columns为[]表示所有都没选
        ...
        let res = await this.fetch_custome_report_detail_table(params);
    ...
    async get_client_name() {
      try {
        ...
        let value = this.$route.query.client;
    ...
    handle_download() {
      let params = {
        ...
        search_end: this.filter.date[1],
        ...this.filter.selects,
        columns: this.filter.selectedColumns,
        perpage: this.pageSize,
        page: this.currentPage
      };
      this.fetch_custome_report_detail_download(params)
        .then(data => {
